...
struct PositionPair {
    ...
    PositionPair(int x, int y) : pointcloud(x), point(y) { }

    PositionPair(const PositionPair & pp) : pointcloud(pp.pointcloud), point(pp.point) { }

    PositionPair() : pointcloud(0), point(0) { }
} ;

class PCDWidget : public QGLWidget {
    Q_OBJECT

public:
    explicit PCDWidget(QWidget *parent = 0);
    explicit PCDWidget(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_ptr, GLuint color = 0xFFFFFFFF);
    virtual ~PCDWidget();
