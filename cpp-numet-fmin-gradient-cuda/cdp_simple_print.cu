...
////////////////////////////////////////////////////////////////////////////////
__device__ void print_info(int depth, int thread, int uid, int parent_uid, int begin)
{
    // only the first thread of each block will print out
    if (threadIdx.x == 0)
    {
        if (depth == 0)
            printf("BLOCK %d launched by the host   - %s\n", uid, begin ? "begin" : "end");
        else
        {
            char buffer[32];
            ...
            buffer[3*depth] = '\0';
            printf("%sBLOCK %d launched by thread %d of block %d   - %s\n", buffer, uid, thread, parent_uid, begin ? "begin" : "end");
        }
    }
...
__global__ void cdp_kernel(int max_depth, int depth, int thread, int parent_uid)
{
    ...
    print_info(depth, thread, s_uid, parent_uid, 1);
    ...
    // RECURSIVE KERNEL CALL
    // only recursive kernel calls can be made, print_info<<<2,2>>> is not valid

    // this kernel call is launched in each thread, consequently every thread will produce further 2x2 nr of threads
    // this kernel call will be launched asynchronously AFTER this kernel returns
    cdp_kernel<<<gridDim.x, blockDim.x>>>(max_depth, depth, threadIdx.x, s_uid);

    // result: further (2x2) x (2x2) = 16 threads

    print_info(depth-1, thread, s_uid, parent_uid, 0);
}
