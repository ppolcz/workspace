...
import polcz.budget.session.AbstractEntityBean;
import polcz.budget.session.AbstractEntityBeanHelper;
import polcz.budget.session.AbstractMapEntityBean;
import polcz.budget.session.ChargeAccountBean;
import polcz.budget.session.ClusterBean;
import polcz.budget.session.MarketBean;
import polcz.budget.session.TransactionBean;
import polcz.budget.session.converter.MapEntityConverter;
import polcz.budget.session.validator.AbstractValidator;
import polcz.budget.session.validator.TransactionValidator;
...
public class ClassDiagramGenerator
{
    ...
    public void generatePlantUML(Class<?>... cs)
    {
        for (Class<?> c : cs)
        {
            GeneratePlantUML(c);
    ...
    public void GeneratePlantUML(Class<?> targetClass)
    {
        ...
        /**
         * Detect parent class if - is not an interface - is not an enum - its
         * super-class is not the Object
         */
        ...
        for (Field f : targetClass.getDeclaredFields())
        {
            ...
            /**
             * Get name of the type of the field: java.lang.String --> String
             */
            ...
            else
            {
                ...
                System.out
                    .println("\t" + ModifierToPlantUML(f.getModifiers()) + f.getName() + " : " + typename);
            }
        }
        ...
        {
            /**
             * TODO - what are these access methods? Are these some significant
            ...
            /**
             * TODO - is it a good idea? Detect instantiation: if a method
             * returns an instance of returnType, it could be an instantiation
             * association.
             */
            // System.out.println(m.getGenericReturnType().toString());
            // if (m.getGenericReturnType().toString().startsWith("class
            // com.napol"))
            // {
    ...
    public static void main(String[] args)
    {
        ...
        // Package p = Package.getPackage("polcz.budget.session");
        //
        // Reflections reflections = new Reflections("polcz.budget.session");
        //
        ...
        // {
        // generator.GeneratePlantUML(DefaultMappableEntity.class);
        // }

        ClassDiagramGenerator generator = new ClassDiagramGenerator();
        generator.addClass(AbstractNameDescEntity.class);

        generator.generatePlantUML(
            AbstractMapEntityBean.class, 
            ChargeAccountBean.class, 
            ClusterBean.class, 
            MarketBean.class,
            TransactionBean.class, 
            AbstractEntityBean.class, 
            AbstractEntityBeanHelper.class,
            AbstractValidator.class,
            TransactionValidator.class,
            // --
            AbstractEntity.class,
            AbstractNameDescEntity.class,
            TChargeAccount.class,
            TCluster.class,
            TMarket.class,
            TTransaction.class,
            TTransactionType.class,
            MapEntityConverter.class,
            // --
