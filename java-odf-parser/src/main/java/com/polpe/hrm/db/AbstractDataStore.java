...
import static com.polpe.hrm.util.Debug.log;
import static com.polpe.hrm.util.Debug.debug;
...
public abstract class AbstractDataStore
{
	...
	/**
	 * @param table
	 * @return drop table MySQL statement for {@param table}
	 */
	private static String sqldroptable(ETableNames table)
	{
		return "DROP TABLE IF EXISTS " + table.sqlname();
	}

	/**
	 * Drop table statements for all tables created by this application
	 * (forced mode: does not check foreign key constraints)
	 * @return string array {@link String[]} of MySQL statements
	 */
	public static String[] sqldroptables()
	{
		ETableNames[] tables = ETableNames.values();
		int l = tables.length;

		String[] ret = new String[l + 2];

		ret[0] = "SET FOREIGN_KEY_CHECKS = 0;";
		for (int i = 0; i < l; ++i)
			ret[i + 1] = sqldroptable(tables[i]);
		ret[l + 1] = "SET FOREIGN_KEY_CHECKS = 1;";
...
//			for (Object[] cl : clsPi) insert(typesPi, cl);
	...
	public static String sqlInsert(IColumn[] c, Object... v)
	{
		...
		String sql = "insert ignore into " + table.sqlname();
		String cols = "";
		String vals = "";
	...
	public static String sqlSelect(String where, IColumn... cols)
	{
		...
		if (tables.size() > 1)
		{
			for (IColumn c : cols)
			{
				...
				whereAdditional += " and " + c.sqlwhere(c.ref().sqlname(), ope);
			}
		}
	...
	public synchronized int insert(IColumn[] c, Object... v)
	{
		...
		{
			System.out.println("Error code = " + e.getErrorCode() + " - " + sqlInsert(c, v));
		}
