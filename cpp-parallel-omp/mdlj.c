...
void xyz_out (FILE * fp,
  ...
  fprintf(fp,"NBODY %22.16g %22.16g\n",step*dt,dt);
  fprintf(fp,"%s\n","ASCII");
  ...
  fprintf(fp,"%s %d %s\n","POINTS",n,"float");
  for (i=0; i<n; i++)
    fprintf(fp,"%22.16g %22.16g %22.16g\n",
...
void init ( double * rx, double * ry, double * rz,
  ...
  if (!vel_ok) {
    for (i=0;i<n;i++) {
      vx[i]=drand48();//gsl_ran_exponential(r,1.0);
      vy[i]=drand48();//gsl_ran_exponential(r,1.0);
      vz[i]=drand48();//gsl_ran_exponential(r,1.0);
    }
  }
...
int main_mdlj ( int argc, char * argv[] ) {
  ...
  sprintf(fn,"output_%06d.vtk",0);
  out=fopen(fn,"w");
  xyz_out(out,rx,ry,rz,vx,vy,vz,ix,iy,iz,L,N,16,1,unfold,0,dt);
  fclose(out);
  ...
  for (s=0;s<nSteps;s++) {
    ...
    if (!(s%fSamp)) {
      sprintf(fn,"output_%06d.vtk",!strcmp(wrt_code_str,"a")?0:s);
      out=fopen(fn,wrt_code_str);
      xyz_out(out,rx,ry,rz,vx,vy,vz,ix,iy,iz,L,N,16,1,unfold,s,dt);
      fclose(out);
    }
  }
}
