 ...
 */
public class TrFormActivity2_UpdateProductInfos extends Activity
{
	interface Operator
	{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		try
		{
			q = getIntent().getExtras().getParcelable(MainActivity3_NoLongPressAction.KEY_TR_QUERY);
			r = q.getFirst();
		...
		try
		{
			c = getIntent().getExtras().getParcelable(MainActivity3_NoLongPressAction.KEY_PI_QUERY);
	...
	private void finishAndReturn()
	{
		...
		IColumn[] insertTypes = { PI_ID, PI_CAID, PI_AMOUNT, PI_CLNAME, PI_DATE, PI_MKID, PI_WHAT };
		int position = 0;
		for (int i = 0; i < pilist.getChildCount(); ++i)
		{
			...
			if (holder != null && holder.id < 0) {
				c.appendRecord(insertTypes, 
					null /* PI_ID */, selectedCaid, getInt(holder.amount), getString(holder.cl), date, selectedMkname, getString(holder.what));
				++position;
			}
			else if (holder != null)
			{
				Util.debug("i = %d + id = %d + what = %s", position, holder.id, getString(holder.what));
				c.setValue(PI_CAID, position, selectedCaid);
				c.setValue(PI_AMOUNT, position, getInt(holder.amount));
				c.setValue(PI_CLNAME, position, getString(holder.cl));
				c.setValue(PI_DATE, position, date);
				c.setValue(PI_MKID, position, selectedMkname);
				c.setValue(PI_WHAT, position, getString(holder.what));
				++position;
			}
		}
	 ...
	 */
	private void addNewPiForm(final ParcelableQuery q, final int position)
	{
		...
		if (q != null && position >= 0)
		{
			holder.amount.setText((CharSequence) q.getValue(PI_AMOUNT, position).toString());
			holder.what.setText((CharSequence) q.getValue(PI_WHAT, position));
			holder.id = (Integer) q.getValue(PI_ID, position);
		}
		...
		setupSpinner(holder.cl, clusters, new Operator()
		{
			...
			{
				String initial;
				if (q != null && position >= 0)	initial = (String) q.getValue(PI_CLNAME, position);
				else initial = getString(cl);
				return row[0].toString().startsWith(initial);
			}
